...
// ── XSS protection : encode les caractères HTML dans les données utilisateur ──
function _h(s) {
...
function printDashboard() {
  ...
  const stockRows = APP.articles.map(a => {
    ...
    return `<tr style="${isAlert?'background:#fff5f5':''}">
      <td>${a.code}</td>
      <td style="font-weight:600">${_h(a.name)}</td>
  ...
  <table>
    ...
    <tbody>${movsP.slice(0,100).map(m=>{
      ...
      return `<tr><td style="white-space:nowrap">${dateStr}</td><td style="color:${isE?'#007700':'#cc4400'};font-weight:700">${isE?'ENTRÉE':'SORTIE'}</td><td>${_h(m.articleName||'—')}</td><td style="font-family:monospace;font-size:10px;color:#555">${art?art.code:'—'}</td><td style="text-align:center;font-weight:700">${isE?'+':'−'}${m.qty}</td><td>${whoLabel}</td><td style="color:#666;font-style:italic">${_h(m.note||m.obs||'')}</td></tr>`;
    }).join('')}
...
function refreshReportCard() {
  ...
  const rows = APP.articles.map(a => {
    ...
    return `<tr style="${isAlert?'background:rgba(255,71,87,.07)':''}">
      ...
      <td style="font-weight:600;font-size:12px">${_h(a.name)}</td>
...
function refreshDashboard(showNotif) {
  ...
  if(alertsEl) alertsEl.innerHTML = alerts.length === 0
    ...
    : alerts.map(a=>`<div class="stat-row"><div><div style="font-size:13px;font-weight:600">${_h(a.name)}</div><div style="font-size:11px;color:var(--text-2)">${a.code}</div></div><div class="text-right"><span class="badge badge-red">Stock: ${a.stock}</span><div style="font-size:11px;color:var(--text-2)">Min: ${a.stockMin}</div></div></div>`).join('');
  ...
  if(actEl) {
    ...
    actEl.innerHTML = recent.length === 0 ? '<div class="empty-state"><p>Aucune activité</p></div>'
      : recent.map(m => {
        ...
        return `<div class="activity-item"><div class="activity-dot" style="background:${isE?'var(--success)':'var(--accent3)'}"></div><div style="flex:1"><div style="font-size:13px"><strong>${isE?'+':'-'}${m.qty}</strong> ${_h(m.articleName)}${whoLabel?' → '+whoLabel:''}</div><div class="activity-meta">${fmtDateTime(m.ts)}</div></div></div>`;
      }).join('');
  }
...
function renderArticleRow(a) {
  ...
  return `<tr id="art-row-${a.id}">
    ...
    <td class="editable" id="td-name-${a.id}">${_h(a.name)}</td>
...
function openArticleModal(id) {
  ...
  <div class="form-row">
    <div class="form-group"><label>Fournisseur</label><select id="f-fournisseur"><option value="">— Aucun —</option>${(APP.fournisseurs||[]).map(f=>`<option value="${f.id}" ${a?.fournisseurId===f.id?'selected':''}>${_h(f.nom)}</option>`).join('')}</select></div>
  </div>
  <div class="form-row">
...
function openManualStockModal(articleId) {
  ...
  const artOptions = APP.articles.map(a =>
    `<option value="${a.id}" ${articleId===a.id?'selected':''}>${_h(a.name)} (stock: ${a.stock})</option>`
  ).join('');
...
function updateManualStockUI() {
  ...
  if(art && prev) {
    ...
    prev.innerHTML = `${icon} <strong>${_h(art.name)}</strong> : ${art.stock} → <strong style="color:${color}">${newStock}</strong>`;
  }
}
...
function saveManualStock() {
  ...
  saveDB();
  closeModal();
  notify(`✅ Stock mis à jour : ${_h(art.name)} → ${art.stock}`, 'success');
  filterArticles();
  updateAlertBadge();
}
...
function openMvtModal(artId) {
  ...
  const body=`
  <div class="form-group"><label>Article</label><input value="${_h(art.name)}" disabled></div>
  <div class="form-row">
  ...
  <div class="form-group"><label>Commercial</label><select id="m-com"><option value="">— Aucun —</option>${APP.commerciaux.map(c=>`<option value="${c.id}">${_h(c.prenom)} ${_h(c.nom)}</option>`).join('')}</select></div>
  ...
  openModal('mvt',`Mouvement — ${_h(art.name)}`,body,()=>{
...
function renderStockPredictions() {
  ...
  const rows = predictions.slice(0,10).map(p =>
    `<div class="stat-row"><div><div style="font-size:13px;font-weight:600">${_h(p.article.name)}</div><div style="font-size:11px;color:var(--text-2)">${p.dailyRate}/jour — Stock: ${p.article.stock}</div></div><div><span class="badge ${p.urgency==='critical'?'badge-red':p.urgency==='high'?'badge-orange':'badge-yellow'}">${p.daysLeft===0?'Rupture!':p.daysLeft+' j'}</span></div></div>`
  ).join('');
...
function openBonModal(bonId) {
  ...
  const coOptions='';
  const comOptions=APP.commerciaux.map(c=>`<option value="${c.id}" ${bon?.commercialId===c.id?'selected':''}>${_h(c.prenom)} ${_h(c.nom)}</option>`).join('');
  const today=new Date().toISOString().split('T')[0];

  let lignesHtml='';
  if(bon && bon.lignes) {
    lignesHtml=bon.lignes.map(l=>{
      const artOptions=[...APP.articles].sort((x,y)=>x.name.localeCompare(y.name,'fr')).map(a=>`<option value="${a.id}" ${a.id===l.articleId?'selected':''}>${_h(a.name)} (stock: ${a.stock})</option>`).join('');
    ...
    const artOpts=[...APP.articles].sort((x,y)=>x.name.localeCompare(y.name,'fr')).map(a=>`<option value="${a.id}">${_h(a.name)} (stock: ${a.stock})</option>`).join('');
...
function generateBonHTML(bon, overrides) {
  ...
  const paperSizeMap={'A4':{rows:16,minHeight:1020},'A5':{rows:6,minHeight:690},'LETTER':{rows:14,minHeight:950},'LEGAL':{rows:21,minHeight:1240}};
  ...
  // Nouvelle sémantique : minRows = TOTAL lignes voulues (vides + articles)
  // Si articles >= total demandé : 0 ligne vide (le tableau s'étend juste avec les articles)
  const _totalRows = !isNaN(_extraRaw) ? _extraRaw : _pinfo.rows;
  const blankCount = Math.max(0, _totalRows - _lignesCount);
  ...
  <div class="table-wrap"><table>
    ...
    <tbody>${mvts.length ? mvts.map(m => {
      ...
      return `<tr>
        ...
        <td style="font-weight:600">${_h(m.articleName||'—')}</td>
...
function openNewMvtModal(type) {
  const artOptions=APP.articles.map(a=>`<option value="${a.id}">${a.code} — ${_h(a.name)} (Stock: ${a.stock})</option>`).join('');
  const comOptions=APP.commerciaux.map(c=>`<option value="${c.id}">${_h(c.prenom)} ${_h(c.nom)}</option>`).join('');
  const foOptions=APP.fournisseurs.map(f=>`<option value="${f.id}">${_h(f.nom)}${f.entreprise?' ('+f.entreprise+')':''}</option>`).join('');
  ...
  ()=>{
    ...
    notify(`${type==='entree'?'Entrée':'Sortie'} de ${qty} × ${_h(art.name)} enregistrée`,'success');
  });
}
    ...
    <div class="card mb-16" style="border-left:4px solid ${color}">
      ...
      <div style="padding:14px 18px">
          ...
          ${zSecteurs.map(s => {
            ...
            <div style="background:var(--bg-2);border:1px solid var(--border);border-left:3px solid ${sColor};border-radius:var(--radius);padding:12px">
              ...
              ${sComs.length ? `<div style="display:flex;flex-wrap:wrap;gap:4px">
                ${sComs.map(c=>`<span style="background:${sColor}22;color:${sColor};border-radius:99px;padding:2px 8px;font-size:10px;font-weight:600;cursor:pointer" onclick="openCommercialModal('${c.id}')" title="Modifier">${_h(c.prenom)} ${_h(c.nom)}</span>`).join('')}
        ...
        <div style="margin-top:10px;padding:10px 12px;background:var(--bg-3);border-radius:var(--radius);border:1px dashed var(--border)">
          ...
          ${zComs.filter(c=>!c.secteurId||!secteurs.find(s=>s.id===c.secteurId&&s.zoneId===z.id)).map(c=>`<span style="background:${color}22;color:${color};border-radius:99px;padding:2px 8px;font-size:10px;font-weight:600;cursor:pointer" onclick="openCommercialModal('${c.id}')">${_h(c.prenom)} ${_h(c.nom)}</span>`).join(' ')}
        </div>` : ''}
      </div>
    </div>`;
  }).join('');
...
function renderPDV() {
  ...
  <div class="filters mb-16">
    ...
    <select id="pdv-com" onchange="_pdvCom=this.value;_renderPDVTable()" style="width:auto">
      ...
      ${APP.commerciaux.map(c=>`<option value="${c.id}"${_pdvCom===c.id?' selected':''}>${_h(c.prenom)} ${_h(c.nom)}</option>`).join('')}
    </select>
...
function _renderPDVTable() {
  ...
  let pdv = (APP.pdv||[]).filter(p => {
    ...
    if(needle) {
      ...
      const s = (APP.secteurs||[]).find(x=>x.id===p.secteurId);
      const c = APP.commerciaux.find(x=>x.id===p.commercialId);
      const hay = _normSearch([p.nom,p.adresse,p.contact,p.tel,p.email,p.type,z&&z.label,s&&s.label,c&&(_h(c.prenom)+' '+_h(c.nom))].filter(Boolean).join(' '));
      if(!hay.includes(needle)) return false;
    }
  ...
  wrap.innerHTML = `<div class="table-wrap"><table>
    ...
    <tbody>${pdv.map(p=>{
      ...
      return `<tr style="${actif?'':'opacity:.5'}${_pdvSelectMode&&_pdvSelected.has(p.id)?';background:rgba(99,102,241,.08)':''}">
        ...
        <td>${c?`<span style="font-size:12px">${_h(c.prenom)} ${_h(c.nom)}</span>`:'<span style="color:var(--text-3)">—</span>'}</td>
...
function openPDVModal(id) {
  ...
  const comOptions = APP.commerciaux.map(c=>`<option value="${c.id}"${p?.commercialId===c.id?' selected':''}>${_h(c.prenom)} ${_h(c.nom)}</option>`).join('');
...
function renderFournRow(f) {
  ...
  return `<tr id="fourn-row-${f.id}">
    <td class="editable" id="td-fname-${f.id}" style="font-weight:600">${_h(f.nom)}</td>
...
function openFournArticlesModal(fournId) {
  ...
  const body = `
    <div style="margin-bottom:12px;font-size:13px;color:var(--text-2)">Fournisseur: <strong style="color:var(--accent)">${_h(f.nom)}</strong> ${f.entreprise?'('+f.entreprise+')':''}</div>
    ...
    <div id="fa-assigned" style="max-height:200px;overflow-y:auto;margin-bottom:16px">
      ${assigned.length?assigned.map(a=>`<div class="fa-item" style="display:flex;justify-content:space-between;align-items:center;padding:6px 10px;background:var(--bg-2);border-radius:6px;margin-bottom:4px;border:1px solid var(--border)">
        <div><span style="font-weight:600">${_h(a.name)}</span> <span style="font-size:11px;color:var(--text-2)">${a.code} — Stock: ${a.stock}</span></div>
    ...
    <select id="fa-select" style="width:100%;margin-bottom:8px">
      ...
      ${unassigned.sort((a,b)=>a.name.localeCompare(b.name,'fr')).map(a=>`<option value="${a.id}">${_h(a.name)} (${a.code})</option>`).join('')}
    </select>
...
function renderGMACard(a, i=0) {
  ...
  return `<div class="gma-art-card" style="animation-delay:${delay}s" onclick="openGMAArticleDetail('${a.id}')">
    <div class="gma-art-img">
      ${a.image?`<img src="${a.image}" alt="${_h(a.name)}">`:`<div class="gma-art-img-ph">📦</div>`}
    ...
    <div class="gma-art-body">
      ${(()=>{const _fr=a.fournisseurId?(APP.fournisseurs||[]).find(x=>x.id===a.fournisseurId):null;return _fr?`<div class="gma-art-fourn">🏭 <strong>${_h(_fr.nom)}</strong></div>`:'<div class="gma-art-fourn" style="opacity:0.55;background:var(--bg-3);border-color:var(--border)">🏭 <strong>Sans fournisseur</strong></div>';})()}
      <div class="gma-art-cat">${a.category}</div>
      <div class="gma-art-name">${_h(a.name)}</div>
...
function openGMAArticleDetail(artId) {
  ...
  <div style="display:grid;grid-template-columns:200px 1fr;gap:20px;align-items:start">
    ...
    <div>
      ...
      <div style="font-size:18px;font-weight:700;margin-bottom:8px">${_h(a.name)}</div>
...
function _auditEntityName(entity, entityId) {
  try {
    switch((entity||'').toLowerCase()) {
      ...
      case 'commercial': return (()=>{const c=APP.commerciaux.find(c=>c.id===entityId);return c?_h(c.prenom)+' '+_h(c.nom):entityId;})();
  ...
  <div style="display:grid;grid-template-columns:320px 1fr;gap:0;height:calc(100vh - 56px);margin:-24px">
    <div style="background:var(--bg-1);border-right:1px solid var(--border);overflow-y:auto;padding:20px;display:flex;flex-direction:column;gap:16px">
      ...
      <div style="background:var(--bg-2);border:1px solid var(--border);border-radius:var(--radius);padding:14px">
        ...
        <div class="form-group"><label>Total lignes du tableau (vides + articles)</label><input type="number" id="be-rows" value="${s.minRows||16}" min="0" max="50" oninput="beLiveUpdate()"></div>
      </div>
      <div style="background:var(--bg-2);border:1px solid var(--border);border-radius:var(--radius);padding:14px">
...
function beLiveUpdate() {
  ...
  s.minRows=(_brv===''||_brv==null)?16:(parseInt(_brv,10)||0);
...
function beSaveToCompany() {
  ...
  APP.settings.bonMinRows=(s.minRows===0||s.minRows===''||s.minRows==null)?(s.minRows===0?0:16):(parseInt(s.minRows,10)||16);
...
function runSmartSearch(q) {
  ...
  APP.commerciaux.forEach(c=>{const name=_h(c.prenom)+' '+_h(c.nom);if(fuzzyMatch(name,q)||fuzzyMatch(c.email||'',q)||fuzzyMatch(c.service||'',q)){const bons=APP.bons.filter(b=>b.commercialId===c.id).length;results.push({type:'commercial',icon:'👤',color:'#00e5aa',title:name,sub:`${c.service||''} · ${bons} bons émis`,action:()=>{closeSmartSearch();showPage('commerciaux');}});}});
